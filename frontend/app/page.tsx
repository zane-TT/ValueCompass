...
function formatBalanceTooltip(params: unknown) {
  ...
  return `
    ...
    ">
      ...
      <div style="margin-bottom: 8px;">
        ${item.marker ?? ""}<b>金额：</b>${value} 亿元
      </div>

      <div style="margin-bottom: 8px;">
        <div style="font-weight: 700; margin-bottom: 2px;">是什么：</div>
        <div>${help?.meaning ?? "暂无说明"}</div>
      </div>

      <div style="margin-bottom: 8px;">
        <div style="font-weight: 700; margin-bottom: 2px;">常见例子：</div>
        <div>${help?.example ?? "暂无举例"}</div>
      </div>

      <div>
        <div style="font-weight: 700; margin-bottom: 2px;">怎么看：</div>
        <div>${help?.watch ?? "暂无分析提示"}</div>
      </div>
    </div>
  `;
    ...
    trendChart.current.setOption(
      {
        ...
        series: [
          ...
          },
          {
            ...
            data: trendData.marketCapLine.map((item) => [
              item.date,
              item.value,
            ]),
          },
  ...
  useEffect(() => {
    if (!peData || !peChart.current) {
      return;
    }

    const dates = peData.peLine.map((item) => item.date);

    const buildHorizontalLine = (value: number) => {
  ...
  async function loadBalanceData() {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/balance?${params.toString()}`
      );
      ...
      if (!response.ok) {
        throw new Error(data.error || "资产负债接口请求失败");
      }
  ...
  async function loadTrendData() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "业绩与市值接口请求失败");
      }
  ...
  async function loadPeData() {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/pe-trend?${params.toString()}`
      );

      const data = (await response.json()) as PeTrendResponse & {
        error?: string;
      };

      if (!response.ok) {
        throw new Error(data.error || "市盈率接口请求失败");
      }
  ...
  async function loadAllData() {
    await Promise.all([loadBalanceData(), loadTrendData(), loadPeData()]);
  }

  const combinedStatus = [balanceStatus, trendStatus, peStatus].join(" | ");
  const combinedError = [balanceError, trendError, peError]
    .filter(Boolean)
    .join(" | ");
    ...
    <main className="page-shell">
      ...
      <section className="panel">
        <div className="chart-card">
          ...
          {peData?.conclusion ? (
            <div className="status">{peData.conclusion}</div>
          ) : null}
        </div>
      </section>
    </main>
